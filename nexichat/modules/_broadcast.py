        ...
        try:
                ...
                for chat in chats:
                    ...
                    try:
                        if broadcast_type == "reply":
                            m = await nexichat.forward_messages(chat_id, message.chat.id, broadcast_content.id)
                        else:
                            m = await nexichat.send_message(chat_id, text=broadcast_content)
                        sent += 1
            ...
            if flags.get("-user", False):
                ...
                for user in users:
                    ...
                    try:
                        if broadcast_type == "reply":
                            m = await nexichat.forward_messages(user_id, message.chat.id, broadcast_content.id)
                        else:
                            m = await nexichat.send_message(user_id, text=broadcast_content)
                        susr += 1
                        ...
                        flood_time = int(e.value)
                        logger.warning(f"FloodWait of {flood_time} seconds encountered for user {user_id}.")
                        if flood_time > 200:
                            logger.info(f"Skipping user {user_id} due to excessive FloodWait.")
                            continue
                        await asyncio.sleep(flood_time)
                    except Exception as e:
